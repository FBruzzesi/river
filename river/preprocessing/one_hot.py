...
class OneHotEncoder(base.MiniBatchTransformer):
    ...
    def transform_many(self, X: IntoDataFrameT) -> IntoDataFrameT:
        ...
        Pandas keeps the historical fast path returning `Sparse[uint8]` columns. Every other
        narwhals-supported backend (polars, pyarrow, ...) is encoded via
        `narwhals.Series.to_dummies` and returns **dense** integer columns, since those backends
        have no sparse-array equivalent.
        ...
        if X_nw.implementation.is_pandas():
            ...
            native = self._transform_many_narwhals(X_nw)
    ...
    def _transform_many_narwhals(self, X_nw: nw.DataFrame[IntoDataFrameT]) -> IntoDataFrameT:
        ...
        # `pandas.get_dummies` omits, so they are dropped below for parity.
        dummies = nw.concat(
            [X_nw[col].to_dummies(separator="_") for col in columns], how="horizontal"
        )
