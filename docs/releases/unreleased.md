...
- `preprocessing.StandardScaler` mini-batch methods (`learn_many`, `transform_many`) now accept and return any [narwhals](https://github.com/narwhals-dev/narwhals)-supported eager backend (pandas, polars, pyarrow, ...) instead of being pandas-only, preserving the input backend (including the pandas index) on output. `transform_many` no longer requires `pandas` to be installed. Output dtypes match the previous pandas implementation for single-dtype batches; a mixed-dtype batch now uses the narrowest float that fits every column (e.g. `float32` + `int16` → `float32`) instead of always falling back to `float64`.
